...
fun main() {
    ...
    val ordering = contents.takeWhile(String::isNotBlank).map {
        val split = it.split("|")
        return@map Pair(split.first().toInt(), split.last().toInt())
    }
    
    val updates = contents.asReversed().takeWhile(String::isNotBlank).asReversed().map {
...
fun getRelevantOrders(ordering: List<Order>, update: Update) = ordering.filter {
...
fun partOne(ordering: List<Order>, updates: List<Update>) = updates.sumOf {
